...
func TestFileServerGeneric(t *testing.T) {
	...
	modifySchedulingManual := gosmapi.Scheduling{
		Strategy: gosmapi.ManualStrategy,
	}
	modifySchedulingCron := gosmapi.Scheduling{
		Strategy: gosmapi.CronStrategy,
	...
	serverNFSmod1, err := client.EditOtherFileServer(context.Background(), serverNFS.ID, mod1)
	if err != nil {
	...
	serverSMBmod2, err := client.EditOtherFileServer(context.Background(), serverSMB.ID, mod2)
	if err != nil {
...
func TestFileServerIntegrated(t *testing.T) {
	...
	serverPowerscaleMod1, err := client.EditIntegratedFileServer(context.Background(), serverPowerscale.ID, mod1)
	if err != nil {
